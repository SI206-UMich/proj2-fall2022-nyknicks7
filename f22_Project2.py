...
.
    ...
    for i in range(len(listing_title)):
        listing_info.append((listing_title[i].text, int(price[i].text.strip('$')), listing_id[i][0]))
    return listing_info
...
def write_csv(data, filename):
    ...
    """
    row = ["Listing Title", "Cost", "Listing ID", "Policy Number", "Place Type", "Number of Bedrooms"]
...
def check_policy_numbers(data):
    ...
    """
    list = "20\d{2}\-00\d{4}STR|STR\-000\d{4}"
    noExemptAndPending = []
    exempt_list = []
    for item in data:
        policyNum = item[3]
        if ((policyNum != "Pending") and (policyNum != "Exempt")):
            print(item[2])
            noExemptAndPending.append(item)
    for item in noExemptAndPending:
        if not re.search(list, item[3]):
            exempt_list.append(item[2])
...
def extra_credit(listing_id):
    ...
    tags = str(soup.find_all('li', class_='_1f1oir5'))

    date_dict = {}
    dates = soup.find_all('li', class_ = '_1f1oir5')
    for date in dates:
        date = date.text.split()
        date_dict[date[1]] = date_dict.get(date[1], 0) + 1
    for num in date_dict.values():
        if num > 90:
            return False
        else:
            return True
